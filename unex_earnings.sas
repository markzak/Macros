...
%if &source eq fundq %then %do;
	...
	proc sql;	
		...
		select a.*, b.datadate as datadate_fundq, b.rdq as &varname format date9.
		from &dsin a left join comp.fundq b
		on a.gvkey eq b.gvkey 
		/* matching datadate will only be true for q4 */
		and a.fyear eq b.fyearq;
	quit;

/*most recent analyst date*/
	proc sql; create table &dsout as 
select 
	a.*, b.anntims, b.anndats, b.ANNDATS_ACT, b.ANNTIMS_ACT,
	/* difference in forecast vs announcement date */
	(b.ANNDATS_ACT-b.anndats) as dat_diff
from 
	aa_1 a, ibes.det_epsus b
where 
	a.ibes_ticker eq b.ticker
	and b.FPI eq '6'
	and a.datadate_fundq - 7 <= b.fpedats <= a.datadate_fundq + 7	
	and b.anndats_act > b.anndats /* or ( b.anndats_act eq b.anndats and b.acttims > b.anntims) */
group by 
	a.gvkey, a.datadate_fundq 
having 
